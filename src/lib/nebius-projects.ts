...
export const NEBIUS_PROJECTS: NebiusProject[] = [
  ...
  },
  {
    ...
    problem:
      "Two open-weight Qwen checkpoints with their refusal behaviour removed. Model 1 is the uncensored one: AEON-7/Qwen3.8-27B-AEON-ULTIMATE-UNCENSORED-BF16, dense, 27B, 55.6 GB of weights. Model 2 is the abliterated one: Youssofal/Qwen3.6-35B-A3B-Abliterated-Heretic-BF16, a mixture of experts, 35B total with about 3B active per token. My MacBook Air M3 has 16 GB of unified memory. It cannot load either one. Model 1 alone needs 70 to 80 GB in BF16, once the KV cache counts. The smaller builds are not an escape: the 4-bit MLX build still wants 32 GB, and a 12 GB GGUF swaps to disk. So I rented a GPU from Nebius.",
    built: [
      "I sized the GPU before I rented anything. One card had to hold model 1 whole, so: gpu-h200-sxm, preset 1gpu-16vcpu-200gb, a single H200 with 141 GB, preemptible at about $2.45 an hour. I skipped the 8-GPU shape, because one GPU holds this model. I skipped the L40S, because 48 GB is not close. The 80 GB H100 was my second choice. The preset name is the trap — that 200 GB is system RAM, not GPU memory. Read it as GPU memory and you rent the wrong machine.",
      "Model 1, AEON-7/Qwen3.8-27B-AEON-ULTIMATE-UNCENSORED-BF16, on vLLM 0.27.1. I set every flag and took no defaults. BF16, a 16384-token context, four concurrent sequences, and 0.85 GPU memory utilization. Then the qwen3 reasoning parser, and automatic tool choice with the qwen3_coder parser. Then the Triton prefill backend for Gated-DeltaNet, and MTP speculative decoding at three draft tokens. The server listened on 127.0.0.1 and I reached it over an SSH tunnel. vLLM binds 0.0.0.0 by default, and a model on a public address has no authentication in front of it.",
      "One error stopped model 1 three times, and it lied about itself. vLLM said it could not examine the Qwen3_5ForConditionalGeneration architecture. That reads like an unsupported model. It was not. Further down the same log, Triton failed to compile a CUDA helper — no Python.h on the machine. python3.12-dev and build-essential fixed it, and vLLM had supported model 1 the whole time.",
      "Model 2, Youssofal/Qwen3.6-35B-A3B-Abliterated-Heretic-BF16, on SGLang: a mixture of experts, 35B total, about 3B active per token, and faster than the dense 27B. I passed on a GGUF build in the same size class — a sidegrade, not a step up. vLLM pulled down 65.39 GiB and 20 of 21 shards, then quit: there is no module or parameter named visual in Qwen3_5Model. The checkpoint carries a vision tower, and vLLM's text loader takes the language experts and refuses the vision weights. SGLang loaded the same repository on the same GPU and the same port, unchanged. I kept vLLM and model 1 in place as the fallback.",
    ...
    outcome: [
      "Then I measured the serving instead of assuming it. I swept concurrency from 1 to 64 on one H200, twice. The first run used the four-sequence limit I shipped. The second used sixty-four. 576 requests, no failures.",
      "The first configuration was leaving the card idle. At four sequences the server pins at about 375 tokens a second. It does that however many callers arrive, so the fifth caller only waits. Time to first token then doubled with each doubling of load: 2.8 seconds, 5.4, 10.8, 21.1.",
      "One flag fixed both halves at once. At 32 concurrent requests, throughput went from 381 to 2,128 tokens a second. Time to first token went from 21.1 seconds to 3.8. That is 5.6 times the work and 5.5 times the speed, from the same GPU. Throughput and latency usually trade against each other. They move together only when the old setting was starving the hardware.",
      "Peak measured throughput is 2,848 tokens a second, at 64 concurrent requests. Per-stream output holds between 87 and 104 tokens a second, up to 16 concurrent. Sixteen people can share the card and not notice that they do.",
      "The next limit is not that flag. vLLM reports a maximum concurrency of 36.94x for 16,384-token requests. That is the KV cache, not the sequence cap. Aggregate throughput was still rising at 64, but only by 34 percent over 32. That is what approaching the wall looks like. Context length and memory use are the next levers.",
      "How it was measured, so it can be checked. The harness runs on the VM against localhost, never through the SSH tunnel. Boston to us-central1 adds more round trip than the quantity I am measuring. Token counts come from the server's own usage block, not from counting chunks. Output is fixed at 256 tokens, at temperature 0. Two warmup requests per run are discarded. Three requests per concurrency level, so this is a measurement and not a benchmark.",
      ...
      "The serving stack is not a free choice. Same weights, same GPU, same port. vLLM refused model 2 and SGLang served it. The cause is a vision tower, riding along in a checkpoint you would file as text-only. Pick the server when you size the deployment, not after the download finishes.",
      "Five of my failures had nothing to do with the models. I used SSH to a 10.x address, which exists only inside the VPC. I made a VM with no public address. I put the SSH key comment in the username field. A form wrapped my public key onto two lines. A bare serve command quietly started Qwen/Qwen3-0.6B, the vLLM default, on 0.0.0.0. The dynamic public address also disappeared the moment I stopped the VM. A new customer can hit any of these before the GPU matters. That is why each one is written down, with its cause and its fix.",
      "Both checkpoints have had their refusal behaviour removed, and that is a class of model you can only run yourself. It is the plainest answer I know to why anyone rents a GPU instead of calling a hosted API. I got it by doing the work, not by arguing the point.",
    ],
  },
  {
    id: "17",
